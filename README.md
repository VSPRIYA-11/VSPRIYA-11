<div align="center">

<!-- PROFILE PHOTO PLACEHOLDER -->
<img src="[https://via.placeholder.com/150/0d1117/00d9ff?text=PHOTO](https://github.com/VSPRIYA-11/VSPRIYA-11/blob/main/SHANMU.png)" width="150" height="150" style="border-radius:50%;" alt="profile photo placeholder"/>

<br/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=32&duration=3000&pause=800&color=00D9FF&center=true&vCenter=true&width=700&lines=SHANMUGA+PRIYA+V;AI%2FML+%E2%80%A2+DATA+SCIENCE+%E2%80%A2+SOFTWARE+DEV" alt="Typing SVG" />

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2500&pause=1000&color=6C7086&center=true&vCenter=true&width=600&lines=AI%2FML+Enthusiast;Data+Science+Student;Software+Developer;AI+Application+Builder;Problem+Solver" alt="Typing SVG" />

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](ADD_LINKEDIN_URL)
[![Email](https://img.shields.io/badge/Email-00D9FF?style=for-the-badge&logo=gmail&logoColor=black)](mailto:ADD_EMAIL)
[![Resume](https://img.shields.io/badge/Resume-1a1a2e?style=for-the-badge&logo=readdotcv&logoColor=00D9FF)](ADD_RESUME_LINK)
[![Portfolio](https://img.shields.io/badge/Portfolio-Coming_Soon-2d2d44?style=for-the-badge&logo=vercel&logoColor=white)](#)

</div>

<div align="center">

`ABOUT` • `SKILLS` • `PROJECTS` • `EXPERIENCE` • `ACHIEVEMENTS` • `STATS` • `CONTACT`
[About](#about-me) · [Skills](#tech-stack) · [Projects](#featured-projects) · [Experience](#experience) · [Achievements](#achievements) · [Stats](#github-stats) · [Contact](#contact)

</div>

<br/>

> Final-year AI & Data Science student building practical AI/ML and software solutions using Python, SQL and modern development tools.

<br/>

## About Me

- 🎓 Final-year B.Tech AI & Data Science student, Velammal Engineering College
- 💡 Interested in AI/ML and Data Science
- 💻 Building practical software applications
- 🧠 Exploring intelligent problem-solving systems
- 📚 Continuously improving technical and analytical skills

<br/>

## Tech Stack

**Languages**

<img src="https://skillicons.dev/icons?i=python,java,javascript,sql,html,css&theme=dark" alt="languages"/>

**AI / ML / Data Science**

![Scikit-learn](https://img.shields.io/badge/scikit--learn-0d1117?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![Pandas](https://img.shields.io/badge/Pandas-0d1117?style=for-the-badge&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/NumPy-0d1117?style=for-the-badge&logo=numpy&logoColor=013243)
![ML](https://img.shields.io/badge/Machine_Learning-0d1117?style=for-the-badge&logo=googleanalytics&logoColor=00D9FF)
![Data Analysis](https://img.shields.io/badge/Data_Analysis-0d1117?style=for-the-badge&logo=googledataflow&logoColor=00D9FF)
![Data Viz](https://img.shields.io/badge/Data_Visualization-0d1117?style=for-the-badge&logo=chartdotjs&logoColor=FF6384)

**Backend**

<img src="https://skillicons.dev/icons?i=flask,nodejs&theme=dark" alt="backend"/>

**Databases**

<img src="https://skillicons.dev/icons?i=mysql&theme=dark" alt="databases"/>

**Tools**

<img src="https://skillicons.dev/icons?i=git,github,vscode,figma&theme=dark" alt="tools"/>

<br/>

## Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🧠 KnowMap — Cross-Domain Knowledge Mapping

AI-powered platform that extracts entities from text and visualizes relationships as an interactive knowledge graph, built during the Infosys Springboard virtual internship.

`Python` `NLP` `Knowledge Graphs` `Gradio`

**Status:** ✅ Complete (Colab-based)

[**View Repository →**](https://github.com/VSPRIYA-11/Knowmap-Cross-Domain-Knowledge-Mapping)

</td>
<td width="50%" valign="top">

### 🩺 Brain MRI Tumor Classification

Academic ML project classifying brain MRI scans (tumor / normal) using OpenCV preprocessing and a Random Forest classifier, wrapped in a Flask web app with role-based dashboards.

`Python` `Flask` `OpenCV` `scikit-learn`

**Status:** ✅ Complete — academic project, not a diagnostic tool

[**View Repository →**](https://github.com/VSPRIYA-11/brain-tumor-detection)

</td>
</tr>
</table>

<details>
<summary><b>🎨 Design Work — Figma Prototype (click to expand)</b></summary>
<br/>

**Voice-Controlled App Interface** — a smart-home voice UI prototype designed during a 4-week UI/UX internship at CodTech IT Solutions.

`Figma` `UI/UX Design`

[**View Repository →**](https://github.com/VSPRIYA-11/FIGMA-DESIGN)

</details>

<br/>

> 🚧 **Voyager AI** — an AI-assisted Smart Travel Companion, currently in development with a teammate. Will be added here once published as a standalone repository.

<br/>

## Experience

<table>
<tr>
<td width="50%" valign="top">

**Infosys Springboard — Virtual Internship**
AI Cross-Domain Knowledge Mapping
8-week virtual internship

</td>
<td width="50%" valign="top">

**CodTech IT Solutions — Internship**
UI/UX Design (Figma) — Voice-Controlled App Interface
4-week internship

</td>
</tr>
</table>

<br/>

## Achievements

🥇 **1st Prize** — Project Expo, Chakravyuha'25
🥇 **1st Prize** — Paper Presentation, TEZAS-2K26
🥉 **3rd Prize** — Pressure & Treasure, TEZAS-2K26

<br/>

## GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=VSPRIYA-11&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00d9ff&icon_color=00d9ff" alt="GitHub Stats" height="165"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=VSPRIYA-11&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00d9ff" alt="Top Languages" height="165"/>

</div>

<br/>

<div align="center">

<!--
SETUP NOTE: The animation below requires a one-time GitHub Actions setup.
Create a repo named exactly "VSPRIYA-11" (a special profile repo) if you don't have one,
then add the workflow from https://github.com/Platane/snk to generate this file automatically.
Until that workflow runs once, this image will not display.
-->
<img src="https://raw.githubusercontent.com/VSPRIYA-11/VSPRIYA-11/output/github-contribution-grid-snake-dark.svg" alt="Contribution Snake"/>

</div>

<br/>

## Contact

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shanmuga-priya-v-124377306?utm_source=share_via&utm_content=profile&utm_medium=member_android)
[![Email](https://img.shields.io/badge/Email-00D9FF?style=for-the-badge&logo=gmail&logoColor=black)](vshanmugapriya2005@gmail.com)
[![Resume](https://img.shields.io/badge/Resume-1a1a2e?style=for-the-badge&logo=readdotcv&logoColor=00D9FF)](https://drive.google.com/file/d/11BhcGEqe1wJlq3oWGgy6QRc9w0fk2isw/view?usp=sharing)

</div>

<br/>

<div align="center">
<sub>Open to AI/ML, Data Science, and Software Development opportunities · 2027 batch</sub>
</div>
